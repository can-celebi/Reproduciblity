...
# Model to analyze - change this to switch between models
...
# Check if model-specific directory exists, fall back to base if not
if (!dir.exists(output_dir)) {
  # Try base directory (for backward compatibility with old structure)
  if (dir.exists(base_output_dir)) {
    output_dir <- base_output_dir
    cat("Note: Using base output directory (no model subdirectory found)\n")
  } else {
...
run_stability_analysis <- function(runs = NULL, force_reload = FALSE) {
  ...
  # Check if results already exist for THIS MODEL and we're not forcing reload
  if (!force_reload && exists("results", envir = .GlobalEnv)) {
    existing <- get("results", envir = .GlobalEnv)
    if (is.list(existing) && all(c("parsed", "stability", "model") %in% names(existing))) {
      if (existing$model == MODEL) {
        cat("Using cached results for", MODEL, "(", nrow(existing$stability), " units from ", 
            max(existing$stability$n_runs), " runs)\n", sep = "")
        cat("To reload, use: results <- run_stability_analysis(force_reload = TRUE)\n\n")
        return(invisible(existing))
      } else {
        cat("Cached results are for", existing$model, "but analyzing", MODEL, "- reloading...\n\n")
      }
    }
  }
...
# Check if results already exist for this model before running
if (exists("results") && is.list(results) && "stability" %in% names(results) && 
    "model" %in% names(results) && results$model == MODEL) {
  cat("Results already loaded for", MODEL, "(", nrow(results$stability), " units from ",
